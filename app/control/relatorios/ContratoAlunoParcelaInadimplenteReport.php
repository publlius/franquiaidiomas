...
class ContratoAlunoParcelaInadimplenteReport extends TPage
{
    private $form; // form
    private $loaded;
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle("Contratos Alunos INADIMPLENTES");
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Contratos Alunos(Inadimplentes)"]));
    ...
    public function onGenerate($format)
    {
        try
        {
            ...
            $filterVar = NULL;
            $criteria->add(new TFilter('data_recebimento', 'is', $filterVar));
            ...
            if ($objects)
            {
                $widths = array(200,200,300,200,200,200,200,200);
                ...
                if (!empty($tr))
                {
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        if ($object->contrato_aluno_id !== $breakValue)
                        {
                            ...
                            $tr->addRow();
                            $tr->addCell($object->render('{id} - {contrato_aluno->curriculo_aluno->aluno->nome} | {contrato_aluno->curriculo_aluno->idioma->descricao}'), 'left', 'break', 8);
                            $breakTotal = [];
                        }
