...
class AulaProfessorReport extends TPage
{
    private $form; // form
    private $loaded;
    ...
    private static $formName = 'form_AulaProfessorReport';
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle("Aulas realizadas");

        $tipo_aula = new TEntry('tipo_aula');

        $tipo_aula->setSize('100%');
        $tipo_aula->setMaxLength(1);

        $row1 = $this->form->addFields([new TLabel("Tipo aula:", null, '14px', null)],[$tipo_aula]);
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Aulas realizadas"]));
    ...
    public function getFilters()
    {
        ...
        if (isset($data->tipo_aula) AND ( (is_scalar($data->tipo_aula) AND $data->tipo_aula !== '') OR (is_array($data->tipo_aula) AND (!empty($data->tipo_aula)) )) )
        {

            $filters[] = new TFilter('tipo_aula', '=', $data->tipo_aula);// create the filter 
        }
        ...
        try
        {
            ...
            if ($objects)
            {
                ...
                if (!empty($tr))
                {
                    ...
                    // add titles row
                    $tr->addRow();
                    $tr->addCell("Tipo aula", 'left', 'title');
