...
class ResponsavelAlunoReport extends TPage
{
    private $form; // form
    private $loaded;
    private static $database = 'cdi';
    private static $activeRecord = 'ResponsavelAluno';
    private static $primaryKey = 'id';
    private static $formName = 'formReport_ResponsavelAluno';
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle("Responsável Aluno");

        $id = new TEntry('id');
        $aluno_id = new TDBUniqueSearch('aluno_id', 'cdi', 'Aluno', 'id', 'nome','nome asc'  );
        $nome = new TDBUniqueSearch('nome', 'cdi', 'ResponsavelAluno', 'id', 'nome','nome asc'  );

        $nome->setMinLength(3);
        $aluno_id->setMinLength(3);

        $nome->setMask('{nome}');
        $aluno_id->setMask('{nome}');
        ...
        $row2 = $this->form->addFields([new TLabel("Aluno:", null, '14px', null)],[$aluno_id]);
        $row3 = $this->form->addFields([new TLabel("Responsável:", null, '14px', null)],[$nome]);
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Responsável Aluno"]));
    ...
    public function getFilters()
    {
        ...
        if (isset($data->aluno_id) AND ( (is_scalar($data->aluno_id) AND $data->aluno_id !== '') OR (is_array($data->aluno_id) AND (!empty($data->aluno_id)) )) )
        {

            $filters[] = new TFilter('aluno_id', '=', $data->aluno_id);// create the filter 
        }
        if (isset($data->nome) AND ( (is_scalar($data->nome) AND $data->nome !== '') OR (is_array($data->nome) AND (!empty($data->nome)) )) )
        {

            $filters[] = new TFilter('id', '=', $data->nome);// create the filter 
        }
    ...
    public function onGenerate($format)
    {
        try
        {
            ...
            $param = [];
            // creates a repository for ResponsavelAluno
            ...
            if ($objects)
            {
                ...
                if (!empty($tr))
                {
                    ...
                    // add titles row
                    $tr->addRow();
                    $tr->addCell("Responsável", 'left', 'title');
                    $tr->addCell("Aluno", 'left', 'title');
                    $tr->addCell("Parentesco", 'left', 'title');
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        $tr->addCell($object->parentesco, 'left', $style);
