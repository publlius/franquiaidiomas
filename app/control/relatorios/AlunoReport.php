...
class AlunoReport extends TPage
{
    private $form; // form
    private $loaded;
    ...
    private static $formName = 'formReport_Aluno';
    ...
    public function __construct()
    {
        ...
        $convenio_id = new TDBCombo('convenio_id', 'cdi', 'Convenio', 'id', '{descricao}','descricao asc'  );
        ...
        $row3 = $this->form->addFields([new TLabel("Situação:", null, '14px', null)],[$situacao_id]);
        $row4 = $this->form->addFields([new TLabel("Convênio:", null, '14px', null)],[$convenio_id]);
        $row5 = $this->form->addFields([new TLabel("Unidade:", null, '14px', null)],[$unidade_id]);
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Alunos"]));
            ...
            if ($objects)
            {
                ...
                if (!empty($tr))
                {
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        $tr->addCell($object->convenio->descricao, 'left', $style);
                        $tr->addCell($object->unidade->name, 'left', $style);
                        $tr->addCell($object->situacao->status, 'left', $style);
