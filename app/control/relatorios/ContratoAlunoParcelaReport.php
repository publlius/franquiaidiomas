...
class ContratoAlunoParcelaReport extends TPage
{
    private $form; // form
    private $loaded;
    ...
    public function __construct()
    {
        ...
        $data_recebimento_ate = new TDate('data_recebimento_ate');
        ...
        $data_recebimento_ate->setMask('dd/mm/yyyy');
        ...
        $data_recebimento_ate->setDatabaseMask('yyyy-mm-dd');

        $id->setSize(100);
        $unidade_id->setSize('100%');
        $data_vencimento->setSize(110);
        $data_recebimento->setSize(110);
        $data_vencimento_ate->setSize(110);
        $contrato_aluno_id->setSize('100%');
        $data_recebimento_ate->setSize(110);
        ...
        $row4 = $this->form->addFields([new TLabel("Recebimento de:", null, '14px', null)],[$data_recebimento],[new TLabel("até:", null, '14px', null)],[$data_recebimento_ate]);
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Contratos Alunos"]));
    ...
    public function getFilters()
    {
        ...
        if (isset($data->data_recebimento) AND ( (is_scalar($data->data_recebimento) AND $data->data_recebimento !== '') OR (is_array($data->data_recebimento) AND (!empty($data->data_recebimento)) )) )
        {

            $filters[] = new TFilter('data_recebimento', '>=', $data->data_recebimento);// create the filter 
        }
        if (isset($data->data_recebimento_ate) AND ( (is_scalar($data->data_recebimento_ate) AND $data->data_recebimento_ate !== '') OR (is_array($data->data_recebimento_ate) AND (!empty($data->data_recebimento_ate)) )) )
        {

            $filters[] = new TFilter('data_recebimento', '<=', $data->data_recebimento_ate);// create the filter 
        }
    ...
    public function onGenerate($format)
    {
        try
        {
            ...
            $filterVar = NULL;
            $criteria->add(new TFilter('data_recebimento', 'is not', $filterVar));
